...
PUBLIC int lottery()
{
	...
	/* This is the basic loop logic for picking a winning process. */
	if (total_tickets > 0) {
			...
			if ((rmp->flags & IN_USE && rmp->is_sys_proc != 1) && rmp->priority == 15) {
				if (winner > 0) {
					...
					if (winner <=0 && rmp->priority == 15) {
						printf("Process:%d won the lottery! tickets:%d/%d priority:%d.\n",
							   rmp->endpoint, rmp->tickets, total_tickets, rmp->priority);
...
PUBLIC int do_stop_scheduling(message *m_ptr)
{
	...
	printf("do_stop_scheduling() called.\n");
...
PUBLIC int do_start_scheduling(message *m_ptr)
{
	...
	printf("do_start_scheduling() called.\n");
...
PRIVATE int schedule_process(struct schedproc * rmp)
{
	...
	printf("schedule_process() called. endpoint:%d priority:%d time_slice:%d tickets:%d\n", rmp->endpoint, rmp->priority,
		   rmp->time_slice, rmp->tickets);
